...
public class movieDescriptionCrawler {
	...
	if (movie_text.hasText() !=  false) {
		...
		this.movie_data.put("movie_text", cleanText(movie_text.first().text().replaceAll("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", "")));
		
		if ( extra_movie_text != null  ) {
			this.movie_data.put("extra_movie_text", cleanText(extra_movie_text.text().replaceAll("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", "")));
		}
		else {
